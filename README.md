...
## 2b. Windows quick start (auto-connect)

Skip steps 2–4 above with two one-time setups already done on this machine:

```bash
# ONE-TIME: install local SSH key on the server (asks for the server
# password once; afterwards tunnels never prompt again). Needs Git Bash.
bash other-scripts/install_ssh_key.sh
```

Then daily usage is just: **double-click `start-admin.bat`** in the project
root — it starts the SSH tunnel (minimized, auto-reconnects, no password),
starts the PHP server on http://localhost:8000, and opens the browser.
Run **`stop-admin.bat`** to shut everything back down — the PHP server, the
SSH tunnel and the browser window it opened.

Under the hood: `other-scripts/tunnel.ps1` (Windows-native rewrite of
`tunnel.sh`, uses key `%USERPROFILE%\.ssh\pinoyride_ed25519`) and
`other-scripts/stop-admin.ps1`.
